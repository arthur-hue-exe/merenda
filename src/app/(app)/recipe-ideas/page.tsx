...
import { Lightbulb, Loader2, Send, User, Bot } from "lucide-react"; // Ícone Lightbulb para ideias
import { getRecipeChatResponse } from "./actions"; 
...
export default function RecipeIdeasPage() {
  ...
  const handleSendMessage = async () => {
    ...
    const chatHistoryForApi = messages.slice(-5); 

    // Usando a action específica para ideias de receitas
    const result = await getRecipeChatResponse(newUserMessage.text, chatHistoryForApi);

    if ("error" in result) {
      toast({ title: "Erro no Chat Culinário", description: result.error, variant: "destructive" });
    ...
    <div className="flex flex-col h-[calc(100vh-10rem)]"> 
      <Card className="flex-1 flex flex-col shadow-lg">
        <CardHeader className="border-b">
          ...
          <CardDescription>
            Converse com a IA para obter ideias, sugestões e inspirações para receitas da merenda escolar.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex-1 overflow-hidden p-0">
          <ScrollArea ref={scrollAreaRef} className="h-full p-4 space-y-3">
        ...
        <CardFooter className="border-t p-4">
          <div className="flex w-full items-center gap-2">
            <Input
              type="text"
              placeholder="Descreva o que você precisa ou pergunte sobre receitas..."
